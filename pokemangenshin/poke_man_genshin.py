...
RARE_PROBABILITY = 0.03  # 戳一戳获得五星卡片的概率
...
sv = Service('poke-man-genshin', bundle='pcr娱乐', help_='''
...
def get_random_cards(origin_cards, card_file_names_list=card_file_names_all, amount=1, bonus=True):
    ...
    for i in range(amount):
        random_card = random.choice(
            card_file_names_list) if card_file_names_list != card_file_names_all else random.choice(
...
async def poke_back(session: NoticeSession):
    ...
    if not daily_limiter.check(guid) or random.random() > POKE_GET_CARDS:
        ...
        card_counter, card_descs, card = get_random_cards(
            db.get_cards_num(session.ctx['group_id'], session.ctx['user_id']), card_file_names_all,
            roll_cards_amount(), True)
        dash = '----------------------------------------'
        msg_part = '\n'.join(card_descs)
        await session.send(f'{card}{at_user}这些卡送给你了, 让我安静会...\n{dash}\n获得了:\n{msg_part}')
